...
void parallelDFS(const std::vector<int>& graph, std::vector<bool>& visited, int startNode) {
    int numNodes = graph.size();
    __m256i stack = _mm256_set1_epi32(startNode); // 使用SIMD向量作为堆栈
    while (!_mm256_testz_si256(stack, stack)) { // 当堆栈非空时
        __m256i currentNodeVec = _mm256_set1_epi32(0);
        _mm256_maskstore_epi32(reinterpret_cast<int*>(&currentNodeVec), _mm256_set1_epi32(-1), stack); // 从堆栈中获取当前节点
        ...
        for (int i = 0; i < 8; ++i) { // 处理SIMD向量中的8个节点
            ...
            if (!visited[currentNode]) {
                ...
                _mm256_maskstore_epi32(reinterpret_cast<int*>(&stack), _mm256_set1_epi32(-1), unvisitedNeighbors);
            }
        }
    }
}
